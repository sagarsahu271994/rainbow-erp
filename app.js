...
function effectiveSupabaseUrl() { return (db.settings && db.settings.supabaseUrl) || SUPABASE_URL; }
function effectiveSupabaseKey() { return (db.settings && db.settings.supabaseKey) || SUPABASE_ANON_KEY; }
...
function safeFileName(value) { return String(value || "document").replace(/[^a-z0-9_-]+/gi, "-").replace(/^-+|-+$/g, "").toLowerCase(); }
function docHeader(title) { return '<div class="doch"><img src="' + logoPath + '" alt=""><div><h2>' + esc(db.settings.name) + '</h2><p>' + esc(db.settings.address) + '</p><b>' + esc(title) + '</b></div></div>'; }
function admissionDocHtml(item) { return docHeader("Admission Form") + '<div class="docgrid"><div><b>Date:</b> ' + esc(item.date) + '</div><div><b>Admission ID:</b> ' + esc(item.admissionId) + '</div><div><b>Student:</b> ' + esc(item.studentName) + '</div><div><b>Class:</b> ' + esc(item.className) + '</div><div><b>Father:</b> ' + esc(item.fatherName) + '</div><div><b>Mother:</b> ' + esc(item.motherName) + '</div><div><b>Mobile:</b> ' + esc(item.mobile) + '</div><div><b>School:</b> ' + esc(item.schoolName || "-") + '</div><div><b>Batch:</b> ' + esc(item.batch || "-") + '</div><div><b>Address:</b> ' + esc(item.address || "-") + '</div></div><div class="signature-row"><div>Parent Signature</div><div>Authorized Signature</div></div>'; }
function receiptDocHtml(item) { return docHeader("Fees Receipt") + '<div class="docgrid"><div><b>Receipt No.:</b> ' + esc(item.receiptNo) + '</div><div><b>Date:</b> ' + esc(item.date) + '</div><div><b>Student:</b> ' + esc(item.studentName) + '</div><div><b>Class:</b> ' + esc(item.className || "-") + '</div><div><b>Month:</b> ' + esc(item.month || (item.date ? item.date.slice(0, 7) : "-")) + '</div><div><b>Payment Mode:</b> ' + esc(item.paymentMode || "Cash") + '</div><div><b>Monthly Fees:</b> ' + rs(item.monthly) + '</div><div><b>Paid Fees:</b> ' + rs(item.fees) + '</div><div><b>Total Paid:</b> ' + rs(item.total) + '</div><div><b>Transaction ID:</b> ' + esc(item.transactionId || "-") + '</div></div><div class="seal">Rainbow<br>Seal</div><div class="signature-row"><div>Received By</div><div>Authorized Signature</div></div>'; }
function openPdfWindow(title, html) { const win = window.open("", "_blank", "width=900,height=1100"); if (!win) { toast("Popup blocked. Browser popup allow karein."); return null; } win.document.write('<!doctype html><html><head><title>' + esc(title) + '</title><link rel="stylesheet" href="styles.css"><style>@page{size:A4;margin:10mm}body{background:#fff}.pdf-page{width:190mm;min-height:270mm;margin:0 auto;padding:0}.doc{display:block!important;border:0;box-shadow:none;padding:0}.actions,.side,.top,.panel{display:none!important}</style></head><body><main class="pdf-page"><section class="doc ready">' + html + '</section></main></body></html>'); win.document.close(); return win; }
function printPdf(title, html) { const win = openPdfWindow(title, html); if (!win) return; setTimeout(() => { win.focus(); win.print(); }, 350); }
async function pdfBlobFromHtml(title, html) {
  ...
  try {
    ...
    return await window.html2pdf().set({ margin: 10, filename: safeFileName(title) + ".pdf", image: { type: "jpeg", quality: 0.98 }, html2canvas: { scale: 2, useCORS: true }, jsPDF: { unit: "mm", format: "a4", orientation: "portrait" } }).from(wrap).outputPdf("blob");
  } finally { wrap.remove(); }
}
async function downloadPdf(title, html, fileName) {
  const blob = await pdfBlobFromHtml(title, html);
  if (!blob) { printPdf(title, html); return; }
...
async function shareDocument(title, html, fileName, text) {
  const blob = await pdfBlobFromHtml(title, html);
  if (!blob) { printPdf(title, html); return; }
  const file = new File([blob], fileName, { type: "application/pdf" });
  if (navigator.share && (!navigator.canShare || navigator.canShare({ files: [file] }))) {
    try { await navigator.share({ title, text, files: [file] }); return; } catch (error) { if (error.name === "AbortError") return; }
  }
  const url = URL.createObjectURL(blob);
  const link = document.createElement("a");
  link.href = url;
  ...
  link.click();
  URL.revokeObjectURL(url);
  toast("PDF download ho gaya. Isse share kar sakte hain.");
}
function renderDocActions(target, item, type, html) { const isAdmission = type === "admission"; const title = isAdmission ? "Admission Form" : "Fees Receipt"; const fileName = safeFileName((isAdmission ? item.admissionId : item.receiptNo) + "-" + item.studentName) + ".pdf"; $(target).insertAdjacentHTML("beforeend", '<div class="actions"><button class="blue" id="' + type + 'Pdf">PDF</button><button class="green" id="' + type + 'Share">Share</button><button id="' + type + 'Wa">WhatsApp</button></div>'); $("#" + type + "Pdf").onclick = () => downloadPdf(title, html, fileName); $("#" + type + "Share").onclick = () => shareDocument(title, html, fileName, title + " - " + (item.studentName || "")); $("#" + type + "Wa").onclick = () => whatsapp(item.mobile, isAdmission ? "Admission Form: " + item.studentName + ", Class " + item.className + ", ID " + item.admissionId : "Receipt " + item.receiptNo + ": " + item.studentName + " ki fees " + rs(item.total) + " receive ho gayi. - Rainbow The Learner Zone"); }
function showAdmissionDoc(item) { const html = admissionDocHtml(item); $("#admissionDoc").className = "doc ready"; $("#admissionDoc").innerHTML = html; renderDocActions("#admissionDoc", item, "admission", html); }
function showReceiptDoc(item) { const html = receiptDocHtml(item); $("#receiptDoc").className = "doc ready"; $("#receiptDoc").innerHTML = html; renderDocActions("#receiptDoc", item, "receipt", html); }
function showModal(title, html) { $("#modalBody").innerHTML = "<h2>" + esc(title) + "</h2>" + html; $("#modal").classList.add("show"); $('[data-profile-admission]').forEach((button) => button.onclick = () => { const item = db.admissions.find((x) => x.id === button.dataset.profileAdmission); if (item) { $("#modal").classList.remove("show"); showAdmissionDoc(item); navigateView("admission", "Admission"); } }); $('[data-profile-fee]').forEach((button) => button.onclick = () => { const item = db.fees.find((x) => x.id === button.dataset.profileFee); if (item) { $("#modal").classList.remove("show"); showReceiptDoc(item); navigateView("fees", "Fees Receipt"); } }); }
function profileHtml(student) { if (!student) return '<div class="empty">Student profile nahi mila.</div>'; const due = nextDueDate(student); const admissions = db.admissions.filter((a) => a.studentId === student.id || normalize(a.admissionId) === normalize(student.admissionId) || normalize(a.studentName) === normalize(student.name)); const receipts = db.fees.filter((fee) => fee.studentId === student.id || normalize(fee.studentName) === normalize(student.name)); const attendance = db.attendance.filter((entry) => entry.studentId === student.id || normalize(entry.student) === normalize(student.name)).sort((a, b) => String(b.date).localeCompare(String(a.date))); const present = attendance.filter((entry) => entry.status === "Present").length; const absent = attendance.filter((entry) => entry.status === "Absent").length; return '<div class="profile-grid"><div><b>Admission ID:</b> ' + esc(student.admissionId) + '</div><div><b>Name:</b> ' + esc(student.name) + '</div><div><b>Class:</b> ' + esc(student.className) + '</div><div><b>Mobile:</b> ' + esc(student.mobile) + '</div><div><b>Admission Date:</b> ' + esc(student.date) + '</div><div><b>Monthly Fees:</b> ' + rs(student.fees) + '</div><div><b>Paid Fees:</b> ' + rs(student.paidFees) + '</div><div><b>Pending Fees:</b> ' + rs(student.pendingFees || Math.max(Number(student.fees || 0) - Number(student.paidFees || 0), 0)) + '</div><div><b>Next Due:</b> <span class="badge">' + esc(fmt(due) || "-") + '</span></div><div><b>Attendance:</b> ' + present + ' Present / ' + absent + ' Absent</div></div><h2 class="profile-section-title">Admission Forms</h2>' + (admissions.length ? admissions.map((a) => '<div class="item"><div><b>' + esc(a.admissionId) + '</b><br><small>' + esc(a.date) + ' | ' + esc(a.className) + '</small></div><button class="ghost" data-profile-admission="' + esc(a.id) + '">Open</button></div>').join("") : '<div class="empty">Admission form record nahi mila.</div>') + '<h2 class="profile-section-title">All Fees Receipts</h2>' + (receipts.length ? receipts.map((fee) => '<div class="item"><div><b>' + esc(fee.receiptNo) + '</b><br><small>' + esc(fee.date) + ' | ' + rs(fee.total) + '</small></div><button class="ghost" data-profile-fee="' + esc(fee.id) + '">Open</button></div>').join("") : '<div class="empty">No receipt found.</div>') + '<h2 class="profile-section-title">Attendance History</h2>' + (attendance.length ? '<div class="tablewrap compact"><table><thead><tr><th>Date</th><th>Status</th><th>Time In</th><th>Time Out</th><th>Remarks</th></tr></thead><tbody>' + attendance.map((entry) => '<tr><td>' + esc(entry.date) + '</td><td>' + esc(entry.status) + '</td><td>' + esc(entry.timeIn || "-") + '</td><td>' + esc(entry.timeOut || "-") + '</td><td>' + esc(entry.remarks || "-") + '</td></tr>').join("") + '</tbody></table></div>' : '<div class="empty">Attendance data nahi mila.</div>'); }
...
function editAttendance(id) { const a = db.attendance.find((x) => x.id === id); if (!a) return; a.status = a.status === "Present" ? "Absent" : "Present"; upsertSupabase("attendance", a); save(); toast("Attendance updated"); }
...
$('#saveSettings').onclick = () => { db.settings = { ...db.settings, ...dataFrom($('#settingsForm')) }; sb = null; saveSettingsSupabase(); save(); loadSupabaseData(); toast('Settings saved'); };
