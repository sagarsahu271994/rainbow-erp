...
async blob(title,html){const wrap=document.createElement("div");wrap.className="pdf-page";wrap.innerHTML='<section class="doc ready">'+html+'</section>';document.body.appendChild(wrap);try{if(!window.html2pdf)return null;return await window.html2pdf().set({margin:10,filename:App.fileName(title)+".pdf",image:{type:"jpeg",quality:.98},html2canvas:{scale:2,useCORS:true},jsPDF:{unit:"mm",format:"a4",orientation:"portrait"}}).from(wrap).outputPdf("blob")}finally{wrap.remove()}},
