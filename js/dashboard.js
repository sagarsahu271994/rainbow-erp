/**
 * App.Dashboard — Rainbow ERP dashboard + pending fees + alerts.
 *
 * Reuses App.Fees for every money calculation (App.Fees._toPaise /
 * App.Fees._fromPaise) and App.Fees.openCollectPopup() for fee collection —
 * no fee math is duplicated in this file. Pending amounts are always
 * derived from fees_receipts (App.db.fees), never from the denormalized
 * student.pendingFees field.
 *
 * No HTML skeleton or CSS changed — all existing #ids (#upcomingDueList,
 * #modal, #modalBody, #pendingFeesDetailsBtn, etc.) and existing action
 * buttons (WhatsApp/Share/Copy/Export PDF/WhatsApp All/Share All/Copy All)
 * are kept exactly as before. New filters (Class, Status) and the Collect
 * Fee button are added only inside the JS-generated modal/card markup that
...
App.Dashboard = {
  ...
  /**
   * FEATURE 10 (performance): groups every fees_receipts row by
   * "studentId|monthKey" ONE time (single pass over App.db.fees) instead of
   * filtering the whole receipts array once per student. Turns pendingRows()
   * from O(students x receipts) into O(students + receipts), which is what
   * makes 10,000+ students / 50,000+ receipts stay responsive.
   * Amounts are summed in paise via App.Fees._toPaise — money math is never
   * duplicated here, it's the same helper fees.js uses.
   */
  _buildReceiptsIndex() {
    const index = new Map();
    App.db.fees.forEach(f => {
      const key = (f.studentId || App.norm(f.studentName)) + '|' + (f.month || App.monthKey(f.date));
      index.set(key, (index.get(key) || 0) + App.Fees._toPaise(f.total));
    });
    return index;
  },

  /**
   * FEATURE 9: pending amount is calculated ONLY from fees_receipts (via the
   * receipts index above), never from student.pendingFees / student.fees
   * directly for "how much is still owed this due month" — the same rule
   * enforced in App.Fees's Collect Fees popup.
   */
  pendingRows() {
    const index = this._buildReceiptsIndex();
    const rows = [];
    App.db.students.forEach(student => {
      const dueDate = App.nextDueDate(student);
      if (!dueDate) return;
      const monthlyPaise = App.Fees._toPaise(student.fees);
      if (!monthlyPaise) return;
      const monthKey = App.monthKey(App.todayFromDate(dueDate));
      const collectedPaise = index.get(student.id + '|' + monthKey) || 0;
      const pendingAmount = App.Fees._fromPaise(Math.max(monthlyPaise - collectedPaise, 0));
      const isPending = pendingAmount > 0 && !App.feePaidForMonth(student, dueDate);
      if (isPending) rows.push({ student, dueDate, pendingAmount, isPending });
    });
    return rows;
  },
  ...
  filterPendingStudents() {
    ...
    const cls = App.$('#pendingClassFilter')?.value || 'all';
    const status = App.$('#pendingStatusFilter')?.value || 'all';
    const today = App.parseDate(App.today());
    // FEATURE 10: filter the already-computed cache (set when the modal
    // opened) instead of recalculating pendingRows() on every keystroke.
    const source = this._pendingRowsCache || this.pendingRows();
    return source.filter(row => {
      const s = row.student;
      const dueMonth = row.dueDate ? App.monthKey(App.todayFromDate(row.dueDate)) : '';
      const overdue = this._isOverdue(row.dueDate, today);
      ...
      const matchesClass = cls === 'all' || App.norm(s.className) === App.norm(cls);
      const matchesStatus = status === 'all' || (status === 'overdue' ? overdue : !overdue);
      const isClosed = this.closedAlerts.has(s.id);
      const matchesAlertState = this.showClosedAlerts ? isClosed : !isClosed;
      return matchesSearch && matchesMonth && matchesClass && matchesStatus && matchesAlertState;
    });
  },
  ...
  /**
   * FEATURE 5: dynamic summary. allRows defaults to the cached full list
   * (set by openPendingFeesModal) instead of recomputing pendingRows()
   * again on every render — same perf reasoning as filterPendingStudents().
   */
  pendingSummary(rows, allRows) {
    const all = allRows || this._pendingRowsCache || this.pendingRows();
    const total = rows.reduce((sum, row) => sum + App.num(row.pendingAmount), 0);
    const allTotal = all.reduce((sum, row) => sum + App.num(row.pendingAmount), 0);
    const today = App.parseDate(App.today());
    const overdue = rows.filter(row => this._isOverdue(row.dueDate, today)).length;
    return { total, allTotal, count: rows.length, allCount: all.length, overdue };
  },

  /** FEATURE 6: dynamic month options, built from whatever due dates actually exist. */
  pendingOptions(rows) {
  ...
  openPendingFeesModal() {
    ...
    App.$('#pendingClassFilter').onchange = () => this.renderPendingTable();
    App.$('#pendingStatusFilter').onchange = () => this.renderPendingTable();
  ...
  renderPendingTable() {
    ...
    const today = App.parseDate(App.today());
    const actionsHtml = (id, closed) => '<div class="pending-row-actions"><button class="green" data-pending-collect="' + id + '">Collect Fee</button><button data-pending-wa="' + id + '">WhatsApp</button><button class="blue" data-pending-share="' + id + '">Share</button><button class="ghost" data-pending-copy="' + id + '">Copy</button><button class="ghost" data-pending-toggle-close="' + id + '">' + (closed ? 'Reopen Alert' : 'Close Alert') + '</button></div>';
    const rowHtml = rows.map(row => {
      ...
      return '<tr><td><b>' + App.esc(s.name) + '</b><br><small>' + App.esc(s.admissionId || '-') + '</small></td><td>' + App.esc(s.className || '-') + '</td><td>' + App.esc(s.mobile || '-') + '</td><td>' + (row.dueDate ? App.fmt(row.dueDate) : '-') + '</td><td><b>' + this.money(row.pendingAmount) + '</b></td><td><span class="status-badge ' + (overdue ? 'overdue' : 'due') + '">' + status + '</span></td><td>' + actionsHtml(id, closed) + '</td></tr>';
    }).join('');
    ...
    App.$$('[data-pending-collect]').forEach(btn => btn.onclick = () => this.collectPending(btn.dataset.pendingCollect));
  ...
  /**
   * FEATURE 3 / 4: opens the shared Collect Fees popup (App.Fees owns all
   * of the logic — validation, Supabase insert, totals refresh, PDF).
   * App.Fees.generateReceipt() already closes #modal and calls App.save()
   * on success, which re-renders App.Dashboard.render() (stats + Alerts)
   * automatically — no page reload, no extra wiring needed here.
   */
  collectPending(id) {
    const s = App.studentById(id);
    if (s) App.Fees.openCollectPopup(s);
  },
  ...
  /**
   * FEATURE 1 — Alert widget (existing #upcomingDueList panel): shows ONLY
   * students whose due date is yesterday, today, or tomorrow, using the
   * App.isDueAlertWindow() helper (utils.js). No arbitrary 8-item cap
   * anymore — every alert in that 3-day window is shown. Accepts an
   * already-computed `pending` array from render() so it never recomputes
   * pendingRows() a second time in the same render pass.
   */
  renderUpcomingDue(pending) {
    const rows = (pending || this.pendingRows()).filter(row => App.isDueAlertWindow(row.dueDate));
    const cards = rows.map(row => {
      const s = row.student;
      const id = App.esc(s.id);
      return '<div class="item due-item"><div><b>' + App.esc(s.name) + '</b><br><small>' + App.esc(s.className) + ' | ' + App.fmt(row.dueDate) + ' | ' + this.money(row.pendingAmount) + '</small></div><div class="due-actions" style="display:flex;gap:8px;flex-wrap:wrap;justify-content:flex-end;"><button class="green" data-due-collect="' + id + '">Collect Fee</button><button data-due-wa="' + id + '">WhatsApp</button><button class="blue" data-due-share="' + id + '">Share</button><button class="ghost" data-due-copy="' + id + '">Copy</button></div></div>';
    });
    App.renderCards('#upcomingDueList', cards, 'No alerts for yesterday, today or tomorrow.');
    App.$$('[data-due-collect]').forEach(b => b.onclick = () => { const s = App.studentById(b.dataset.dueCollect); if (s) App.Fees.openCollectPopup(s); });
  ...
  render() {
    ...
    // FEATURE 5 / 9 / 10: computed once here, reused for both the summary
    // cards below AND the Alerts widget — never recomputed twice per render.
    ...
    this.renderUpcomingDue(pending);
